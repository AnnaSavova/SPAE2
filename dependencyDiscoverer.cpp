/*
 * Student Name:Fanghui Li
 * Student ID: 2431253L
 * title of the assignment: SP exercise 2 
 *
...
int CRAWLER_THREADS = 1; // crawler threads(1,2,3,4,6,8)
...
struct workQStruct{
    ...
    public:
      void push_back(std::string string){
        std::unique_lock<std::mutex> lock(mutex); // unique lock
...
struct theTableStruct{
    ...
    public:
      std::list<std::string> * find(std::string name){
        std::unique_lock<std::mutex> lock(mutex); // unique lock
        return &Table[name]; // std::list<std::string> *ll = &theTable[name];
      }
      ...
      bool checkTable(std::string filename){
        return Table.find(filename) != Table.end();
      }
}; typedef theTableStruct theTableStruct;

workQStruct workQ;
theTableStruct theTable;
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if(theTable.checkTable(name)){continue;}
...
static void printDependencies(std::unordered_set<std::string> *printed,std::list<std::string> *toProcess,FILE *fd){
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = theTable.find(name); // std::list<std::string> *ll = &theTable[name];
...
int main(int argc, char *argv[]) {
  ...
  for(i=0; i < CRAWLER_THREADS; i ++){
    std::thread thread_temp = std::thread([](){
      std::string filename = workQ.pop_front();
      while (filename != "NULL") {
        if (theTable.checkTable(filename) == false) {
          fprintf(stderr,"Mismatch between table and workQ\n");
        }
        process(filename.c_str(),theTable.find(filename)); // 4a&b. lookup dependencies and invoke 'process'
        filename = workQ.pop_front();
      }
    });
    temp.push_back(std::move(thread_temp));
  }
  workQ.setfinished();
  for(std::thread& thread_temp:temp) thread_temp.join(); // thread safety
